...
try{
 ...
 await page.getByRole('tab',{name:'Reference PDFs'}).click();
 ...
 await page.getByRole('tab',{name:'Questions & deductions'}).click();
 await page.locator('.crop-surface canvas[data-ready="true"]').waitFor();
 await page.locator('.zoom-actions').getByRole('button',{name:'Crop answer',exact:true}).click();
 await page.locator('.crop-surface canvas[data-ready="true"]').waitFor();
 ...
 await page.reload();await page.locator('.solution-excerpt').nth(1).locator('canvas[data-ready="true"]').waitFor();
 assert.equal(await page.locator('.solution-excerpt').count(),2,'Crops survive remote save and reload');
 await page.getByRole('button',{name:'+ Question',exact:true}).click();
 await page.locator('[data-edit="question"][data-q="1"][data-field="title"]').waitFor();
 const q=page.locator('.question-editor');
 await q.locator('[data-field="prompt"]').fill('Explain your reasoning.');
 await q.locator('[data-field="assignmentPages"]').fill('1');
 await q.locator('[data-field="solutionPages"]').fill('2');
 await q.locator('[data-field="expected"]').fill('A justified conclusion.');
 await q.getByRole('button',{name:'Add criterion'}).click();
 await q.locator('[data-edit="criterion"][data-field="label"]').fill('Justify the result');
 await q.locator('[data-edit="band"][data-field="label"]').first().click();
 await page.locator('.save-state').filter({hasText:'saved to course'}).waitFor();
 ...
 assert((await page.locator('.course-rail').boundingBox()).height<300,'Mobile course navigation stays compact');
