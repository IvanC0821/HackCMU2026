...
try{
 ...
 await page.goto(`${origin}/teacher/#/homework/1`);
 await page.locator('.question-chart').waitFor();
 ...
 await studentPage.goto(`${origin}/student/`);
 await studentPage.getByRole('button',{name:'Upload your work'}).waitFor();
 assert.equal(await studentPage.locator('h1').innerText(),state.course);
 const studentText=await studentPage.locator('#main').innerText();
 assert(!/Your assignments|Published standard|Published homework|Student view|Connected to your course/.test(studentText));
 await studentPage.screenshot({path:'/private/tmp/verity-clean-student.png',fullPage:true});
