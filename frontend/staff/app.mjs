...
function render(focus = false) {
  ...
  if (connected && ['dashboard', 'review', 'submissions'].includes(ui.route)) {
...
async function refreshWorkspace() {
  ...
  try {
    ...
    if (fresh?.revision > state.revision) { rememberGradeDraft(); state = fresh; persistedRevision = fresh.revision; ui.storageError = ''; ui.storageStatus = connected ? 'Connected · live updates' : 'Saved in this browser'; render(); announce('Course activity updated.'); }
