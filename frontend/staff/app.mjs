...
function setDocURLs() {
  ...
  for (const doc of docs) if (!ui.docURLs[doc.id]) ui.docURLs[doc.id] = doc.sample ? doc.path : doc.blob ? URL.createObjectURL(doc.blob) : '';
}
...
function save() {
  ...
  ui.storageStatus = 'Saving locally…';
  saveChain = saveChain.then(async () => {
    if (blocked) return;
    await store.save(snapshot, persistedRevision); persistedRevision = snapshot.revision;
    ui.storageStatus = 'Saved in this browser'; channel?.postMessage({revision: persistedRevision});
...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    if (action === 'publish') { const version = publishDraft(state); ui.message = `Standard v${version.id} finalized locally. Existing reviews keep their original standard.`; location.hash = '#/homework/1'; }
    ...
    if (action === 'complete') { completeReview(state, ui.sid); ui.message = 'Review completed locally. No real grade was published.'; }
...
channel?.addEventListener('message', async () => {
  if (!store || ui.busy || state.revision !== persistedRevision || document.activeElement?.matches('input, textarea, select')) return;
  const fresh = await store.load();
  if (fresh?.revision > state.revision) { state = fresh; persistedRevision = fresh.revision; render(); announce('Workspace updated from another tab.'); }
});
async function start() {
  render();
  try {
    store = await openStore(); const restored = await store.load();
    ...
    ui.storageStatus = 'Saved in this browser';
  } catch (error) { ui.storageError = error.message; ui.storageStatus = 'Session only'; }
