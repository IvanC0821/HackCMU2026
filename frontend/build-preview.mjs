...
const modules = await Promise.all(['staff/model.mjs', 'staff/row-check.mjs', 'staff/case.mjs', 'staff/storage.mjs', 'staff/api.mjs', 'staff/case-view.mjs', 'staff/view.mjs', 'staff/app.mjs'].map(read));
const js = 'const connected = false;\n' + modules.map(source => source.replace(/^import .+;\n/gm, '').replace(/^export /gm, '')).join('\n');
