...
function header(){
 ...
 ${isReview?`<div class="estimate"><div><span>Estimated grade</span><strong>${state.result?.estimatedScore!=null?`${esc(state.result.estimatedScore)}<small> / ${esc(state.result.maxScore)}</small>`:'<span class="score-wait">Pending</span>'}</strong></div><span class="estimate-caption">Not official</span></div>`:`<span class="student-label">Student view</span>`}</header>`;
}
...
function feedbackBody(){
 ...
 ${findings.length?findings.map(f=>`<button class="feedback-card ${state.activeFinding===f.id?'selected':''}" data-finding="${esc(f.id)}" aria-pressed="${state.activeFinding===f.id}"><span class="feedback-card-top"><span class="deduction-square" aria-hidden="true">${f.applied?'✓':findingNumber(state.result.findings,f.id)}</span>${Number.isFinite(f.deduction)&&f.deduction>=0?`<span class="applied-deduction"><span class="sr-only">Estimated deduction </span>−${f.deduction}</span>`:''}<strong>${esc(f.category)}</strong></span><span class="feedback-message">${esc(f.message)}</span><span class="feedback-location">${located(f)?`View on page ${f.pageIndex+1}`:esc(locationLabel(f))} ${located(f)?icon('arrow'):''}</span></button>`).join(''):`<p class="no-deductions">${score==null?'Feedback for this question is pending.':deduction>0?'A deduction breakdown is not available yet.':'No deductions suggested.'}</p>`}`;
}
function review(){
 ...
 <aside class="feedback-panel" aria-label="Questions and estimated deductions"><div class="student-sidebar-heading"><h1>Questions</h1><span>Estimated grades · not official</span></div>
 ...
 <div class="review-footer"><span>${icon(state.busy?'clock':'check')}${state.busy?esc(state.progress):`Version ${state.revision?.number||1} saved ${state.storageWarning?'for this session':'locally'}`}</span><span>Estimated feedback · not an official grade</span></div></div>`;
}
